...
app = modal.App("ecc-training")
volume = modal.Volume.from_name("ecc-training-data", create_if_missing=True)
ROOT_DIR = Path("/data")
...
image = (
    ...
    .apt_install("git")
    .pip_install("torch", "torchaudio", "torchvision", "galois", "numpy", "tqdm")
    .add_local_dir("./codes", remote_path=CODES_DIR)
)
...
@app.function(gpu="t4", volumes={ROOT_DIR: volume}, image=image, timeout = 24 * 60 * 60)  
def _train(args):
    epochs = args.epochs
    ...
    if not os.path.exists(CODES_DIR / code):
        print(f"ERROR -- please specify a valid code, path '{CODES_DIR / code}' does not exist")
        os._exit(1)
        
    dataset = LLRCodeDataset(CODES_DIR / code, split="train", epoch_len=batch_size * epoch_len, EbN0_min=args.snr_min, EbN0_max=args.snr_max) 
    test_dataset = LLRCodeDataset(CODES_DIR / code, split="val", epoch_len=100, EbN0_min=args.snr_min, EbN0_max=args.snr_max)
    ...
    if args.model == 'transformer':
        model_id = f'experiment_{code[:-4]}_{args.model}_d={args.transformer_dim}_N={args.transformer_layers}_{"mask" if args.mask else "nomask"}_lr={args.learning_rate}_epochs={args.epochs}'
    else:
    ...
    torch.save(model.state_dict(), MODELS_DIR / f'{model_id}.pth')
    print("Done training!")
    volume.commit()
